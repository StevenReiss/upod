...
ConditionConflict getConflicts(Condition c);
...
/**
 *	poll to check if the condition holds in a given state.
 **/

boolean poll(World state) throws ConditionException;
...
void addHandler(ConditionHandler hdlr);
...
void removeHandler(ConditionHandler hdlr);
